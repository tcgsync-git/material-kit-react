...
const Page = () => {
  ...
  const [bottomNavValue, setBottomNavValue] = useState("platformconfig");
        ...
        try {
          ...
          if (existingRarityRules) {
            ...
            selectedGame.uniqueRarities.forEach(rarity => {
              if (auth.user.rarityRules[selectedGame.categoryId.toString()][rarity] !== undefined) {
          ...
          {selectedGame && (
            <Grid container spacing={3} mt={3}>
              <Grid item xs={12} md={6}>
                <Typography variant="h6">{`Rarity: ${selectedGame.displayName}`}</Typography>
                <Table>
                  <TableHead>
                    <TableRow>
