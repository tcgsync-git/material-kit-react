...
const Page = () => {
    ...
    const [selectedPricePoint, setSelectedPricePoint] = useState('lowPrice');

    const handlePricePointChange = (event) => {
        setSelectedPricePoint(event.target.value);
    };
    ...
    const exportCSV = () => {
        const csvData = products.flatMap(product =>
            product.skus.map(sku => ({
                ...
                price: sku.printingName == "Normal" ? product.usPrices[0].lowPrice : product.usPrices[1].lowPrice
            }))
        );
            ...
            <Box component="main" sx={{ flexGrow: 1, py: 8 }}>
                <Container maxWidth="xl">
                    ...
                    <Grid container spacing={3}>
                        ...
                        <Grid item xs={12} md={6}>
                            <FormControl fullWidth>
                                <InputLabel id="price-point-select-label">Select Price Point</InputLabel>
                                <Select
                                    labelId="price-point-select-label"
                                    id="price-point-select"
                                    value={selectedPricePoint}
                                    onChange={handlePricePointChange}
                                >
                                    <MenuItem value="lowPrice">Low Price</MenuItem>
                                    <MenuItem value="midPrice">Mid Price</MenuItem>
                                    <MenuItem value="highPrice">High Price</MenuItem>
                                    <MenuItem value="marketPrice">Market Price</MenuItem>
                                    <MenuItem value="directLowPrice">Direct Low Price</MenuItem>
                                </Select>
                            </FormControl>
                                    ...
                                    {currentProducts.map((product) => (
                                        product.skus.map((sku, index) => (
                                            <TableRow key={`${product.productId}-${sku.skuId}`}>
                                                ...
                                                <TableCell>{product.usPrices[0][selectedPricePoint]}</TableCell>
                                            </TableRow>
                                        ))
                                    ))}
                                </TableBody>
